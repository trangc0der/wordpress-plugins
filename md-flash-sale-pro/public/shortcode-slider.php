...
/**
 * Thu thập sản phẩm đang active trong slot
 */
...
add_shortcode('md_flash_sale_slider', function($atts=array()){
  ...
  $ids = array(); 
  foreach($map as $k=>$r){ 
    $ids[] = $r['vid'] ? $r['vid'] : $r['pid']; 
  }

  // Query cả product & variation
  $q = new WP_Query(array(
  ...
  // Lấy end gần nhất cho countdown
  $nearest_end = 0; 
  foreach($map as $r){ 
    $t=$r['slot_end']; 
    if(!$nearest_end || ($t && $t<$nearest_end)) $nearest_end=$t; 
  }
  if(!$nearest_end) $nearest_end = time() + 3600; // fallback 1h
  ...
  <div class="mdfs-tabs-wrap mdfs-slider-wrap">
    <div class="mdfs-section-head">
      <div class="mdfs-title">
        ...
        <div class="mdfs-countdown" id="mdfs-slider-countdown" data-end="<?php echo esc_attr($nearest_end); ?>">
